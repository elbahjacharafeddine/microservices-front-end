...
import $ from 'jquery'; 
import PDFNewAss from './PDFNewAss';
...
function getStepContent(step,props) {
  switch (step) {
    case 0:
      return <AddressForm year={props.year} month={props.mounth} montant={props.montant} type={props.type} id={props.name}/>;
    default:
...
export default function CheckoutAssurance(props) {
  ...
  const payeFacture = async()=>{
    ...
    axios({
        ...
        url: "http://192.168.48.31:8085/"+user.id+"/assurance/paye facture/"+props.name,
        method: "GET",
        })
          ...
          {activeStep === steps.length ? (
            <React.Fragment>
              ...
              <Button
                ...
                <PDFNewAss facture = {facture}/></Grid>
            </React.Fragment>
          ) : (
            <React.Fragment>
